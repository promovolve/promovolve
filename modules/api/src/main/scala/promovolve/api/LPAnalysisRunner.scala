...
object LPAnalysisRunner {
  ...
  def apply(
    ...
    def persist(result: LPAnalysisResult, captured: Map[String, promovolve.browser.LPCapturedImage])
        : Future[LPAnalysisResult] = {
    ...
    (req: LPWorker.AnalyzeLP) => {
      ...
      lpAnalyzer.analyze(req.url, req.strategy, onScreenshot)
        .flatMap { case (result, captured) =>
          persist(result, captured.images).flatMap(persistFonts(_, captured))
        }
        .flatMap { finished =>
