...
final class ServeRoutes(
                        ...
                        case Some(cand) =>
                          ...
                          for {
                            click <- clickUrl(req.pub, req.url, outcome.slotId.value, cand.creativeId.value, version,
                              cand.campaignId.value, cand.advertiserId.value, cand.category.value, outcome.requestId, apc, None)
                            imp <- impUrl(
                              req.pub, req.url, outcome.slotId.value, cand.creativeId.value, version,
                              cand.campaignId.value, cand.advertiserId.value, cpmDollars, cand.category.value, outcome.requestId, apc, None,
                            )
                            ...
                            foldToken <- foldTokenFor(req.pub, req.url, outcome.slotId.value, cand.creativeId.value, version,
  ...
  private def clickUrl(pub: String, url: String, slot: String, cid: String, ver: Long,
    ...
    signedUrl("click", pub, url, slot, cid, ver, Some(campaignId), Some(advertiserId), None, Some(category), Some(requestId), adProductCategory, pageCategories)

  private def ctaUrl(pub: String, url: String, slot: String, cid: String, ver: Long,
      campaignId: String, advertiserId: String, category: String, requestId: String,
      adProductCategory: Option[String], pageCategories: Option[String]): Future[Option[String]] =
    signedUrl("cta", pub, url, slot, cid, ver, Some(campaignId), Some(advertiserId), None, Some(category), Some(requestId), adProductCategory, pageCategories)
  ...
  private def impUrl(
    ...
    signedUrl("imp", pub, url, slot, cid, ver, Some(campaignId), Some(advertiserId), Some(cpm), Some(category), Some(requestId), adProductCategory, pageCategories)
}
