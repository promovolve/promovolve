...
object LPWorker {
    ...
    if (!config.hasPath("promovolve.browser.lp-workers")) base
      ...
      base.copy(
        ...
        maxConcurrentPerWorker =
          if (w.hasPath("max-concurrent-per-worker")) math.max(1, w.getInt("max-concurrent-per-worker")) else base.maxConcurrentPerWorker,
        staleAfter =
    ...
    def grant(req: AnalyzeLP): Unit = {
      ...
      ctx.pipeToSelf(runAnalysis(req)) {
        ...
        case Failure(e) =>
          AnalyzeFinished(
            req.url,
            AnalyzeLPDone(req.url, LPAnalysisResult(req.url, Vector.empty), None, Some(e.getMessage)),
          )
      }
      log.info(
  ...
  def initKeepAlive(
    ...
    LoggerFactory.getLogger("promovolve.LPWorker")
      .info("LPWorker keep-alive daemon initialised (numWorkers={}, every {})", settings.numWorkers, settings.keepAliveInterval)
  }
