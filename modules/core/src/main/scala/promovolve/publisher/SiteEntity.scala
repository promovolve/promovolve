...
import org.apache.pekko.http.scaladsl.model.{ headers, HttpMethods, HttpRequest, StatusCodes, Uri }
...
object SiteEntity {
        ...
        .setup[Command] { ctx =>
          ...
          def categoryOptimizer(category: String, state: State): FloorSweepOptimizer =
            floorSweepOptimizerByCategory.getOrElse(category, {
                ...
                ctx.log.info("SiteEntity {} per-category sweep optimizer created for category={}", siteId.value,
                  category)
                opt
              })
          ...
          def observeAllCategories(state: State)
              : (Map[String, CPM], Map[String, FloorSweepOptimizer.Snapshot],
                  Vector[(String, FloorSweepOptimizer.CycleDecision)]) = {
            ...
            floorSweepOptimizerByCategory.foreach { case (cat, opt) =>
              ...
              if (isZeroDemand) {
                ...
                if (!prev.contains(floor)) {
                  decisions = decisions :+
                    (cat -> FloorSweepOptimizer.CycleDecision(
                      argmaxFloor = floor,
                      prevArgmax = prev,
                      cycleRevenue = 0.0,
                      cycleImps = 0L,
          ...
          def refreshDemandCategories(): Unit = {
            ...
            ctx.pipeToSelf(
              campaignDirectory.ask[CampaignDirectory.AllCategoriesResult](CampaignDirectory.GetAllCategories(_)).map(
                _.categories)
            ) {
            ...
            command match {
              ...
              case Initialize(config, replyTo) =>
                if (state.isInitialized) {
                  ...
                  val newState =
                    state.withPublisherId(config.publisherId).withConfig(config).withVerificationToken(token)
                ...
                Effect
                  ...
                  .thenRun { _ =>
                    ...
                    if (adjustedState.floorCpm != state.floorCpm) {
                      ...
                      publishPacingConfig(state.pacingConfig.copy(bidWeight = adjustedState.bidWeight,
                        floorCpm = adjustedState.floorCpm.toDouble))
                    }
                  }
              ...
              case UpdateBidWeight(weight, replyTo) =>
                ...
                Effect
                  .persist(newState)
                  .thenRun(_ =>
                    publishPacingConfig(state.pacingConfig.copy(bidWeight = clamped,
              ...
              case GetVerificationStatus(replyTo) =>
                Effect.none.thenReply(replyTo)(_ =>
                  VerificationStatusResult(
                    siteId, state.isVerified, state.verifiedHost,
                    state.verificationToken.map(_.value)
                  ))
              ...
              case RequestVerification(replyTo) =>
                (state.verificationToken, state.config) match {
                  case (Some(token), Some(config)) =>
                    ...
                    ctx.log.info("SiteEntity {} verifying host '{}' via {}", siteId.value, host,
                      candidates.mkString(", "))
                        ...
                        response.status match {
                          ...
                          case status if status.isRedirection && hopsLeft > 0 =>
                            ...
                            response.header[headers.Location] match {
                              ...
                              case Some(_) =>
                                Future.successful(
                                  Left(s"HTTP ${status.intValue()} with relative Location (not followed)"))
                              case None =>
                  ...
                  case _ =>
                    Effect.none.thenReply(replyTo)(_ =>
                      VerificationNotReady(siteId, "Site not initialized or no verification token"))
                }
              ...
              case GetRecentFloorObservations(limit, replyTo) =>
                Effect.none.thenReply(replyTo)(_ =>
                  FloorObservationsResult(recentFloorObservations.take(math.max(0, limit))))
  ...
  final case class SlotFloorOverrideUpdated(siteId: SiteId, slotId: String, floor: Option[CPM])
      extends promovolve.CborSerializable
   ...
   */
  final case class UpdateAcceptsFillerTraffic(accept: Boolean, replyTo: ActorRef[AcceptsFillerTrafficUpdated])
      extends Command
  final case class AcceptsFillerTrafficUpdated(siteId: SiteId, acceptsFillerTraffic: Boolean)
      extends promovolve.CborSerializable
  ...
  final case class AdProductCategoriesBlocked(siteId: SiteId, blocked: Set[AdProductCategoryId])
      extends promovolve.CborSerializable
  ...
  final case class AdProductCategoriesUnblocked(siteId: SiteId, unblocked: Set[AdProductCategoryId])
      extends promovolve.CborSerializable
  ...
  final case class AdProductBlocklistResponse(siteId: SiteId, categories: Set[AdProductCategoryId])
      extends promovolve.CborSerializable
