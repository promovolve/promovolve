...
class PerCategoryEnforceVerificationSpec extends AnyWordSpec with Matchers {
  ...
  "Per-category floor enforcement (3 campaigns at $1/$2/$3)" should {
    ...
    "price each category at its OWN floor under ENFORCE, not the single site floor" in {
      ...
      sb.append(f"    ${"OFF (one site floor)"}%-22s ${fmt(off(0)._2)}%-12s ${fmt(off(1)._2)}%-12s ${fmt(off(2)._2)}%-12s  $$${off.map(_._2).sum}%.2f\n")
      sb.append(f"    ${"ENFORCE (per-category)"}%-22s ${fmt(enforce(0)._2)}%-12s ${fmt(enforce(1)._2)}%-12s ${fmt(enforce(2)._2)}%-12s  $$${enforce.map(_._2).sum}%.2f\n")
      println(sb.toString)
    ...
    "DELIVERY proof (starvation regime): a high single floor starves $1/$2; enforce fills all" in {
      ...
      sb.append(f"    ${"OFF (one site floor)"}%-22s ${fmt(off(0)._2)}%-14s ${fmt(off(1)._2)}%-14s ${fmt(off(2)._2)}%-14s\n")
      sb.append(f"    ${"ENFORCE (per-category)"}%-22s ${fmt(enforce(0)._2)}%-14s ${fmt(enforce(1)._2)}%-14s ${fmt(enforce(2)._2)}%-14s\n")
      println(sb.toString)
