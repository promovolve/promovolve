#!/usr/bin/env bash
# Approve only the final, CI-generated pin-back PR after both pin jobs finish.
set -euo pipefail

mode="${1:?usage: approve-pin-back-pr.sh find|approve}"
: "${GITHUB_REPOSITORY:?}"
...
require_stale_dismissal() {
  ...
  ' <<<"$rules" >/dev/null \
    || { echo "Main must dismiss stale reviews before App auto-approval" >&2; return 1; }
}
...
case "$mode" in
  ...
  approve)
    ...
    ;;
  *)
    echo "usage: approve-pin-back-pr.sh find|approve" >&2
    exit 2
    ;;
esac
